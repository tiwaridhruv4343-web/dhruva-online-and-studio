...
function renderSecurity(data){
  ...
  $("#aSecurityEvents").textContent=(data.events||[]).length;
  $("#aSecurityBlocks").textContent=(data.blocks||[]).length;
  $("#aFailedLogins").textContent=counts.LOGIN_FAILED||0;
  $("#aHoneypot").textContent=counts.HONEYPOT_TRIGGER||0;
  $("#adminSecurity").innerHTML=(data.events||[]).map(v=>`<tr><td><b>${esc(v.event_type)}</b></td><td>${esc(v.route)}</td><td>${esc(v.details||"")}</td><td>${fmt(v.created_at)}</td></tr>`).join("")||`<tr><td colspan="4">No security events recorded.</td></tr>`;
}
